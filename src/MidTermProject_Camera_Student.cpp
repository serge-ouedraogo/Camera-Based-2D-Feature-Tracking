...
int main(int argc, const char *argv[])
{
    ...
    for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex++)
    {
        ...
        {
          cout << " The Buffer was empty: Inserting first element" << endl;
          //If the buffer is empty insert first element at back_idx =0
          front_idx =0;
          back_idx = 0;
          //dataBuffer[back_idx] = frame;
          dataBuffer.push_back(frame);
        }
        
        //If the buffer is full
        if((back_idx+1)%dataBufferSize == front_idx)
        {
          //cout << "the Buffer is full ...please dequeue before inserting" <<endl;
          front_idx = (front_idx + 1)%dataBufferSize;
          back_idx = (back_idx + 1)%dataBufferSize;
          //dataBuffer[back_idx] = frame;
          dataBuffer.push_back(frame);
        }
        
        //If the buffer is not empty and is not full then insert an element
        else
        {
          //cout << "Filling up the buffer" <<endl;
          back_idx = (back_idx + 1)%dataBufferSize;
          //dataBuffer[back_idx] = frame;
          dataBuffer.push_back(frame);
        }
      
      //dataBuffer = RingBuffer(frame)
        //dataBuffer.push_back(frame);
      ...
      if (bFocusOnVehicle)
      {
        ...
        if (dataBuffer.size() > 1) // wait until at least two images have been processed
        {
          ...
          descriptor_time = (double)cv::getTickCount(); 
          ...
          descriptor_time = ((double)cv::getTickCount() - descriptor_time) / cv::getTickFrequency();
      ...
      total_time += detection_time + descriptor_time;
    } // eof loop over all images

    std::cout << "*******************************" << std::endl;
    std::cout << "Total number of Keypoints = " << Num_keypoints << std::endl;
